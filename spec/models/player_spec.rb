...
RSpec.describe Player, :type => :model do
  ...
  let!(:finished_match) { FactoryBot.create(:match, finished: true) }

  describe "Scope 'confirmed_match(id)'" do
    it "Return true or false if is confirmed in a certain match" do
      player = FactoryBot.create(:player)
      presence = FactoryBot.create(:presence,  player_id: player.id, match_id: unfinished_match.id, confirmation: false )
    ...
    describe "Scope 'goals_match(match_id)'" do
      ...
      it "Return the number of gols the player scored on that match" do
        presence = FactoryBot.create(:presence,  player_id: player.id, match_id: finished_match.id, goals: 3, presence: true)
    ...
    describe "Scope 'assists_match(match_id)'" do
      ...
      it "Return the number of assists the player scored on that match" do
        presence = FactoryBot.create(:presence,  player_id: player.id, match_id: finished_match.id, assist: 2, presence: true)
    ...
    describe "Scope payed_match(match_id)" do
      it "Return true or false if the player payed the match" do
        ...
        presence = FactoryBot.create(:presence,  player_id: player.id, match_id: finished_match.id, goals: 3, payed: true)
    ...
    describe "Scope team_played(match_id)" do
      it "Return the team id or nil (when someone does not register the team)" do
        ...
        presence = FactoryBot.create(:presence,  player_id: player.id, match_id: finished_match.id, presence: true, team_id: 1)
    ...
    describe "Scope players_by_season(season_id)" do
      ...
      before do
        @first_player = FactoryBot.create(:player)
        @second_player = FactoryBot.create(:player)

        @first_season = FactoryBot.create(:season)
        @second_season = FactoryBot.create(:season)

        @first_season.matches << FactoryBot.create_list(:match, 5 , { season_id: @first_season.id })
        @second_season.matches << FactoryBot.create_list(:match, 5 , { season_id: @second_season.id })

        @first_season.matches[1..2].each do |match|
          FactoryBot.create(:presence, match: match, player: @first_player )
          FactoryBot.create(:presence, match: match, player: @second_player )
        end
        FactoryBot.create(:presence, match: @first_season.matches[3], player: @second_player )

        @second_season.matches[1..3].each do |match|
          FactoryBot.create(:presence, match: match, player: @first_player )
          FactoryBot.create(:presence, match: match, player: @second_player )
        end
        FactoryBot.create(:presence, match: @second_season.matches[4], player: @second_player )
      end
    ...
    describe "Scope best_team(matches)" do
      it "must group teams that scored the most and less" do
        player = FactoryBot.create(:player)
        good_player = FactoryBot.create(:player)
        bad_player = FactoryBot.create(:player)
        first_match =  FactoryBot.create(:match, finished: true)
        second_match =  FactoryBot.create(:match, finished: true)
        #first match (good team)
        FactoryBot.create(:presence,  player_id: player.id, match_id: first_match.id, goals: 2, presence: true, team_id: 2)
        FactoryBot.create(:presence,  player_id: good_player.id, match_id: first_match.id, goals: 6, presence: true, team_id: 2)
        #second match (bad team)
        FactoryBot.create(:presence,  player_id: player.id, match_id: second_match.id, goals: 2, presence: true, team_id: nil)
        FactoryBot.create(:presence,  player_id: bad_player.id, match_id: second_match.id, goals: 0, presence: true, team_id: nil)
